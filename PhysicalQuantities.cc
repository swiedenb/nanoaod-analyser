...
float transverse_mass(	const float& part_pt, 
						const float& part_phi, 
						const float& met_pt, 
						const float& met_phi) {
	return sqrt(2*part_pt*met_pt*cos(part_phi*met_phi));
};

// Calculate MT from first (highest pt) particle, which fulfils mask, -  and MET
float transverse_mass2(	const rvec<float>& part_pt, 
						const rvec<float>& part_phi, 
						const rvec<bool>& part_mask, 
						const float& met_pt, 
						const float& met_phi) {
	return sqrt(2*part_pt[part_mask][0]*met_pt*cos(part_phi[part_mask][0]*met_phi));
};

// Calculate invariant mass from two particles
float invariant_mass(	const float& part1_pt,
...
float delta_phi(	const float& part1_phi,
					const float& part2_phi) {
	double dphi = std::abs(part1_phi - part2_phi);
	if (dphi > M_PI)
		dphi -= M_PI;
	return dphi;
};
