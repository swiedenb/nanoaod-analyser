...
float dd_fakerate( const rvec<float>& tau_pt,
  ...
  if( tau_pt[tau_mask][col_idx[0]] < 150.){
      if( abs(tau_eta[tau_mask][col_idx[0]]) < 1.446){
          ...
          FF_closure_2  = config::ff_closure_hist->GetBinContent(tauptbin,tauptojetptbin);
      ...
      if( abs(tau_eta[tau_mask][col_idx[0]]) < 1.446){
          ...
          FF_closure  = config::ff_closure_hist_high->GetBinContent(tauptbin,tauptojetptbin);
      }
      else{
          auto tau_pt_value = tau_pt[tau_mask][col_idx[0]];
          if( tau_pt_value > 999.){
