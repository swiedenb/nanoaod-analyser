...
class TauESTool {
    ...
    public:
      ...
      TauESTool(const std::string& year, const std::string& id);
      ~TauESTool() { }
      
      float getTES(const float& pt, const int& dm, const int& genmatch = 5, const std::string& unc = "") const;
      float getTES_highpt(const float& pt, const int& dm, const int& genmatch = 5, const std::string& unc = "") const;
};
#endif // TauIDSFTool_h
