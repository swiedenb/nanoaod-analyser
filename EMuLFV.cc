...
void create_hists(	RNode df,
    ...
    //    
    //}
	auto tau_pt = df.Histo1D(	{((TString) "Muon_pt" + stringcopy), "", 					6000u, 0, 6000}, 		"sel_Muon_pt", 				weight_column);
	auto tau_eta = df.Histo1D(	{((TString) "Muon_eta" + stringcopy), "", 					100u, -5, 5}, 			"sel_Muon_eta", 				weight_column);
	auto tau_phi = df.Histo1D(	{((TString) "Muon_phi" + stringcopy), "", 					100u, -3.2, 3.2}, 		"sel_Muon_phi", 				weight_column);
	auto sphericity = df.Histo1D(	{((TString) "Sphericity" + stringcopy), "", 					100u, 0, 1.}, 		"Sphericity", 				weight_column);
	auto sphericity_leptons = df.Histo1D(	{((TString) "Sphericity_leptons" + stringcopy), "", 					100u, 0, 1.}, 		"Sphericity_leptons", 				weight_column);
	auto muon_pt = df.Histo1D(	{((TString) "Electron_pt" + stringcopy), "", 					6000u, 0, 6000}, 		"sel_Electron_pt", 				weight_column);
...
void fill_stage_with_syst(  RNode df, 
    ...
    if (!config::runOnData) {
        ...
        auto split_true = df.Filter([](){
            return false;
        }, {});
        
        auto split_false = df.Filter([](){
            return true;
        }, {});
...
RNode calc_datadriven(RNode df,
    ...
    auto delR_cut = diele_cut.Filter(mutaudelR_cut,{"sel_Muon_eta","sel_Muon_phi", "sel_Tau_eta", "sel_Tau_phi"},"Mu Tau Del R Cut");
...
//  auto eventcounter = df.Count();
    ...
    auto df_final = [&]() -> RNode{
        ...
        auto evntselect = masked.Filter(nparticle_cut, {"Electron_mask"}, "select_eles")
                                .Filter(nparticle_cut, {"Muon_mask"}, "select_muons");
        
        
        auto df_col_idx = evntselect.Define("col_idx", inv_idx, {"Muon_pt_corr" + muon_shift, "Electron_pt" + ele_shift, "Muon_eta", "Electron_eta", "Muon_phi", "Electron_phi", "Muon_mass", "Electron_mass" + ele_shift, "Muon_mask", "Electron_mask"});   
        // Now that we have our real tau, write it to special columns to handle it more easily
        auto defquants = df_col_idx.Define("sel_Muon_PF_pt", selected_part_col_idx_muon, {"Muon_pt", "Muon_mask", "col_idx"})
                                   .Define("sel_Muon_isPF", selected_part_col_idx_muon_bool, {"Muon_isPFcand", "Muon_mask", "col_idx"})
                                   .Define("sel_Muon_pt", selected_part_col_idx_muon, {"Muon_pt_corr" + muon_shift, "Muon_mask", "col_idx"})
                                   .Define("sel_Muon_eta", selected_part_col_idx_muon, {"Muon_eta", "Muon_mask", "col_idx"})
                                   .Define("sel_Muon_phi", selected_part_col_idx_muon, {"Muon_phi", "Muon_mask", "col_idx"})
        ...
        if (!config::runOnData) {
            defquants = defquants.Define("sel_Electron_genIdx", selected_part_col_idx_muon_idx,{"Electron_genPartIdx","Electron_mask","col_idx"});
            defquants = defquants.Define("sel_Muon_genIdx", selected_part_col_idx_muon_idx,{"Muon_genPartIdx","Muon_mask","col_idx"});
            defquants = defquants.Define("CollMass_gen", [](const rvec<float>& genmuonpt,
                                                            const rvec<float>& genmuoneta,
                                                            const rvec<float>& genmuonphi,
                                                            const rvec<float>& genmuonmass,
                                                            const int& Electron_genIdx,
                                                            const int& Muon_genIdx){
                                            TLorentzVector ele, muon;
                                            muon.SetPtEtaPhiM(genmuonpt[Muon_genIdx],genmuoneta[Muon_genIdx],genmuonphi[Muon_genIdx],genmuonmass[Muon_genIdx]);
                                            muon.SetPtEtaPhiM(genmuonpt[Electron_genIdx],genmuoneta[Electron_genIdx],genmuonphi[Electron_genIdx],genmuonmass[Electron_genIdx]);
                                            double mass_vis = (ele + muon).M();
                                            if (mass_vis != mass_vis) mass_vis=0;
                                            if (mass_vis <= 0.) mass_vis = 0.;
                                            return mass_vis;

                                 
                                 },{"GenPart_pt","GenPart_eta","GenPart_phi","GenPart_mass","sel_Electron_genIdx","sel_Muon_genIdx"})
                                           .Define("CollMass_gen_sel", [](
                                                        ...
                                                        TLorentzVector ele, muon, highest_ele, highest_muon;
                                                        float highest_mass = 0;
                                                        int best_i = 0;
                                                        int best_j = 0;
                                                        for (unsigned int i = 0; i < genmuonpt.size(); i++){
                                                                if( abs(genmuonpdgid[i]) == 11 and abs(genmuonpdgid[genmuonmotheridx[i]]) == 32){
                                                                    ele.SetPtEtaPhiM(genmuonpt[i], genmuoneta[i], genmuonphi[i], genmuonmass[i]);
                                                                    double mass_vis = 0;
                                                        ...
                                                        return highest_mass;
                                             },{"GenPart_pt","GenPart_eta","GenPart_phi","GenPart_mass","GenPart_pdgId","GenPart_genPartIdxMother","GenPart_statusFlags"});
        }
        
        //auto diele_cut = defquants.Filter(dielepair_cut,{"Electron_pt" + ele_shift,"Electron_eta","Electron_phi", "DiElectron_mask"},"Di Ele Pair Cut");
        auto delR_cut = defquants.Filter(mutaudelR_cut,{"sel_Electron_eta","sel_Electron_phi", "sel_Muon_eta", "sel_Muon_phi"},"Mu Muon Del R Cut");
        ...
        auto df_coll = delR_cut.Define("CollMass", mass_inv, {"sel_Muon_pt", "sel_Electron_pt", "sel_Muon_eta", "sel_Electron_eta", "sel_Muon_phi", "sel_Electron_phi", "sel_Muon_mass", "sel_Electron_mass"});  
        auto df_mll = df_coll;
            ...
            total_weights = total_weights.Define("MuonISOScaleFactor", [](const rvec<float>& muon_pt,
                                    ...
                                    .Define("TriggerScaleFactor", [](const rvec<float>& pt,
                                                                     ...
                                                                     const rvec<bool>& mask
                                                                         ...
                                                                         return (float) 1.;
                                                                    }, {"Muon_pt_corr"+ muon_shift, "Muon_eta", "HLT_Photon200", "Muon_mask"})
                                    .Define("TriggerScaleFactorUp", [](const rvec<float>& pt,
                                                                     const rvec<float>& eta,
                                                                     const bool& triggered,
                                                                     const rvec<bool>& mask
                                                                         ...
                                                                         return (float) 1.;
                                                                    }, {"Muon_pt_corr", "Muon_eta", "HLT_Photon200", "Muon_mask"})
                                    .Define("TriggerScaleFactorDown", [](const rvec<float>& pt,
                                                                     const rvec<float>& eta,
                                                                     const bool& triggered,
                                                                     const rvec<bool>& mask
        ...
        auto df_mt = total_weights.Define("Sphericity",sphericity_full,{"Muon_pt_corr" + muon_shift,"Muon_eta","Muon_phi","Muon_mass","Electron_pt" + ele_shift,"Electron_eta","Electron_phi","Electron_mass","Jet_pt","Jet_eta", "Jet_phi", "Jet_mass","Muon_mask", "Electron_mask"}).Define("Sphericity_leptons",sphericity_leptons,{"Muon_pt_corr" + muon_shift,"Muon_eta","Muon_phi","Muon_mass","Electron_pt" + ele_shift,"Electron_eta","Electron_phi","Electron_mass","Muon_mask", "Electron_mask"});
