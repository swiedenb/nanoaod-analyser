...
ROOT::VecOps::RVec< ROOT::VecOps::RVec< float > > calc_tau_energy_scale(const rvec<int>& tau_decayMode,
        ...
        for (uint i = 0; i<tau_pt.size(); i++) {
            ...
            if (config::tau_energy_scale_hist == NULL)
                std::cout << "No histogram for energy scale correction found!" << std::endl;
                p1 *= 1;
            auto hbin = (config::tau_energy_scale_hist)->GetXaxis()->FindBin( tau_decayMode[i] );
            if (config::run_type == "_TauEnergyScaleUp")
                p1 *= (config::tau_energy_scale_hist)->GetBinContent( hbin ) + (config::tau_energy_scale_hist)->GetBinErrorUp( hbin );
            else if (config::run_type == "Tau_EnergyScaleDown")
                p1 *= (config::tau_energy_scale_hist)->GetBinContent( hbin ) + (config::tau_energy_scale_hist)->GetBinErrorLow( hbin );
            else
                p1 *= (config::tau_energy_scale_hist)->GetBinContent( hbin );
